...
Transmission.prototype.get = function(ids, callback) {
	...
	var self = this;
	self.callServer(options, callback)
}
Transmission.prototype.peers = function(ids, callback) {
	...
	var self = this;
	self.callServer(options, function(err, result) {
		if (err) {
			return callback(err)
		}
...
Transmission.prototype.callServer = function(query, callBack) {
		...
		function onEnd() {
			if (response.statusCode == 409) {
				...
				if (json.result === 'success') {
					...
					callBack(new Error(json.result))
				}
			}
		}
	...
	http.request(options, onResponse).on('error', function(err) {
		callBack(err);
